...
import src.Chroma_consultant
from src.Chroma_consultant import get_empathy_context
...
class PromptChatbot(Chatbot):
    ...
    def reset(self):
        ...
        if gender == '남성':
            if age >= 10:
                image_path = 'boy_10_above.webp'
            else:
                image_path = 'boy_10_below.webp'
        else:
            if age >= 10:
                image_path = 'girl_10_above.webp'
            else:
                image_path = 'girl_10_below.webp'
...
def load_persona_from_file():
    persona = {}
    try:
        with open('./data/shared_persona.txt', 'r', encoding='utf-8') as file:
            for line in file:
                key, value = line.strip().split(': ', 1)
                persona[key] = value
    except FileNotFoundError:
...
def save_persona_to_file(persona):
    try:
        with open('./data/shared_persona.txt', 'w', encoding='utf-8') as file:
...
def chat_langchain(user_input, history):
    ...
    try:
        ...
        if gender == '남성':
            if age >= 10:
                image_path = './assets/boy_10_above.webp'
            else:
                image_path = './assets/boy_10_below.webp'
        else:
            if age >= 10:
                image_path = './assets/girl_10_above.webp'
            else:
                image_path = './assets/girl_10_below.webp'
...
chatbot2 = PromptChatbot(model_name='gpt-4o', persona_file='./data/shared_persona2.txt')
    ...
    with gr.Tab("멋진 아빠 되기 프로젝트"):
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                if gender == '남성':
                    if age >= 10:
                        initial_image_path = './assets/boy_10_above.webp'
                    else:
                        initial_image_path = './assets/boy_10_below.webp'
                else:
                    if age >= 10:
                        initial_image_path = './assets/girl_10_above.webp'
                    else:
                        initial_image_path = './assets/girl_10_below.webp'
        ...
        with gr.Row():
            ...
            cb_send_btn = gr.Button(
                value="보내기",
                icon="free-icon-conversation-1206850.png",
                scale=1,
                variant="primary"
            )
        ...
        with gr.Row():
            gr.Button(value="마지막 대화 저장", icon='./assets/free-icon-txt-file-2267023.png').click(
