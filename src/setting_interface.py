...
from display_interface import app2, handle_event
...
def counseling_bot_chat(message, persona, chat_history):
    ...
    try:
        ...
        if not chat_history['introduced']:
            ...
            # 상황 부여
            age, chat_history['history'], _ = handle_event(chat_history['age'], chat_history['history'])
...
with gr.Blocks(theme=gr.themes.Base()) as app:
    ...
    with gr.Tabs() as tabs:
        with gr.TabItem("아이 페르소나 설정", id="persona"):
            gr.Markdown(
                value="""
                # <center>아이 페르소나 설정</center>
                <center>자녀의 페르소나를 설정하고 챗봇과 대화를 시작하세요.</center>
                """
            )
            with gr.Row():
                with gr.Column():
                    name = gr.Textbox(label="이름", lines=1, placeholder="이름(닉네임)", scale=1)
                    age = gr.Slider(label="나이", minimum=5, maximum=15, step=1, value=5)  # 나이 슬라이더 범위 수정
                    gender = gr.Radio(label="성별", choices=["남성", "여성"])
                    personality = gr.Textbox(label="성격", lines=1, placeholder="성격")
                    hobbies = gr.Textbox(label="취미", lines=1, placeholder="취미")
                    speaking_style = gr.Textbox(label="말투", lines=1, placeholder="말투", scale=5)
                    parent_role = gr.Radio(label="부모 역할", choices=["엄마", "아빠"])

            gr.Button(value="페르소나 설정 완료").click(fn=set_persona, inputs=[
